...
const LOG_PREFIX = '[PacificaCandles]';

// Represents the params needed to subscribe to a Pacifica candle stream
...
export class PacificaCandlesClient {
    ...
    // Maps app-level asset keys to Pacifica symbols
    ...
    connect(onBars: (bars: OhlcvBar[]) => void, onConnection: ConnectionStatusCallback) {
        if (this.ws) {
            this.closeSocket();
        }
        this.onBars = onBars;
    ...
    private openSocket(): void {
        ...
        this.ws.onmessage = (event) => {
            ...
            if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
                console.warn(`${LOG_PREFIX} skip non-object message`);
                return;
            }

            const msg = parsed as { channel?: string; data?: unknown };

            // Pacifica responds to our { method: 'ping' } with { channel: 'pong' } — silently ignore
            if (msg.channel === 'pong') {
                return;
            }
            ...
            if (msg.channel === 'candle') {
                ...
                try {
                    ...
                    const filtered = raw.filter((c: unknown) => {
                        const candle = c as { s?: string; i?: string };
                        return (
                            !this.activeSub ||
                            (candle.s === this.activeSub.symbol && candle.i === this.activeSub.interval)
                        );
                    });
                    bars = normalizePacificaCandlesChannelData(filtered);
                } catch (err) {
    ...
    private applySubscription(symbol: string, interval: string): void {
        if (this.activeSub && this.activeSub.symbol === symbol && this.activeSub.interval === interval) {
            return;
        }

        this.sendUnsubscribeIfNeeded();

        this.activeSub = { source: 'candle', symbol, interval };
        this.send({
            method: 'subscribe',
            params: this.activeSub,
        });
    }
