...
export const feedToastId = {
    ...
    hlCandlesHistory: (asset: string, tf: string) => `feed:hl-history:${asset}:${tf}`,
...
export function toastFeedError(id: string, title: string, description?: string): void {
    toast.error(title, {
        id,
...
export function toastFeedWarning(id: string, title: string, description?: string): void {
    toast.warning(title, {
        id,
        description: description ? truncate(description) : undefined,
        duration: 4_000,
